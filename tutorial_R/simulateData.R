...
# to calculate log-normal parameters
...
simdat<-data.frame(c(rep('Group1',times=n),rep('Group2',times=n)),simdat)
colnames(simdat)<-c('group', 'score')

# calculate summary stats

summary_simdat <- summarySE(simdat, measurevar="score", groupvars=c("group"))
